...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json().catch(() => ({}))) as RemoveRolesPayload
    ...
    const result = await fetcher<{
      ...
      message?: string
      data?: string
    }>('/api/org/team/section/users/roles/remove', {
      method: 'POST',
      headers: {
    ...
    const message =
      err?.response?.data?.message ?? err?.message ?? 'Failed to remove roles'
