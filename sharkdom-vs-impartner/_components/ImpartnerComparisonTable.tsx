...
const ROWS: {
  ...
  {
    title: 'Partner Onboarding & Training',
    subtitle: 'Track completion of modules before allowing deal registrations',
    impartner: {
      kind: 'text',
      value: 'Complex LMS integrations',
      color: '#3E424D'
    },
    sharkdom: {
      kind: 'text',
      value: 'Native LMS with quizzes + auto-approval',
      color: '#0D8236'
    }
  },
  {
    title: 'Deal Registration & Attribution',
    subtitle: 'Attribution workflows and pipeline visibility',
    impartner: { kind: 'check' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Account Mapping & Overlaps',
    subtitle: 'Identify mutual opportunities and target account lists',
    impartner: { kind: 'cross' },
    sharkdom: {
      kind: 'text',
      value: 'Built-in co-selling & privacy-first mapping',
      color: '#0D8236'
    }
  },
  {
    title: 'CRM & Ecosystem Sync',
    subtitle: 'Keep direct sales and partner pipelines aligned',
    impartner: { kind: 'check' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Time to setup & go-live',
    subtitle: 'From initial contract to active partner registrations',
    impartner: { kind: 'text', value: 'Weeks to months', color: '#3E424D' },
    sharkdom: { kind: 'text', value: 'Under 14 days' }
  },
  {
    title: 'Free Trial',
    subtitle: 'Test out functionality before purchasing',
    impartner: { kind: 'cross' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Pricing Model',
    subtitle: 'Cost scaling with partner count and tier levels',
    impartner: {
      kind: 'text',
      value: '~5x pricing (Enterprise seats)',
      color: '#3E424D'
    },
    sharkdom: { kind: 'text', value: 'Flat pricing with unlimited partners' }
  }
]
...
export default function ImpartnerComparisonTable() {
  return (
    <section
            ...
            {ROWS.map((row) => (
              ...
              >
                ...
                <div className='flex items-center justify-center border-l border-[#e2e4eb] px-3 py-4'>
                  <CellContent cell={row.impartner} />
                </div>
