...
const ROWS: {
  ...
  {
    title: 'AI-driven partner recruiting',
    subtitle: 'Auto-source qualified partners from the open web',
    crossbeam: { kind: 'cross' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Account mapping & overlap',
    subtitle: 'Find shared customers with partners',
    crossbeam: { kind: 'check' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Partner onboarding workflows',
    subtitle: 'Automated training, certification, tier-ups',
    crossbeam: { kind: 'text', value: 'Limited' },
    sharkdom: { kind: 'text', value: 'Full LMS included' }
  },
  {
    title: 'Multi-touch attribution',
    subtitle: 'Credit every partner touch on every deal',
    crossbeam: { kind: 'text', value: 'Last-touch only' },
    sharkdom: { kind: 'text', value: 'Multi-touch, every channel' }
  },
  {
    title: 'MDF & co-op marketing',
    subtitle: 'Budget, approve and track marketing funds',
    crossbeam: { kind: 'cross' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Per-partner permissions',
    subtitle: 'Fine-grained internal access per partner',
    crossbeam: { kind: 'text', value: 'Workspace-level' },
    sharkdom: { kind: 'text', value: 'Per partner, per field' }
  },
  {
    title: 'Time to first value',
    crossbeam: { kind: 'text', value: '~42 days' },
    sharkdom: { kind: 'text', value: 'Under 7 days' }
  },
  {
    title: 'Pricing model',
    crossbeam: { kind: 'text', value: 'Per-partner seat' },
    sharkdom: { kind: 'text', value: 'Flat + unlimited partners' }
  },
  {
    title: 'Partner verification & compliance',
    subtitle: 'Automated due diligence and tier readiness',
    crossbeam: { kind: 'cross' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Global partner marketplace',
    subtitle: 'Discover and recruit net-new partners',
    crossbeam: { kind: 'cross' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Deal desk & pipeline hygiene',
    subtitle: 'Structured approvals and shared visibility',
    crossbeam: { kind: 'text', value: 'Spreadsheet handoffs' },
    sharkdom: { kind: 'text', value: 'Native deal rooms' }
  },
  {
    title: 'CRM integrations',
    subtitle: 'Bidirectional sync depth',
    crossbeam: { kind: 'text', value: 'Strong Salesforce focus' },
    sharkdom: { kind: 'text', value: '29+ native integrations' }
  },
  {
    title: 'Affiliate & referral payouts',
    subtitle: 'Pay partners accurately at scale',
    crossbeam: { kind: 'cross' },
    sharkdom: { kind: 'check' }
  },
  {
    title: 'Executive reporting',
    subtitle: 'Pipeline and influenced revenue in one view',
    crossbeam: { kind: 'text', value: 'Mapping-centric reports' },
    sharkdom: { kind: 'text', value: 'Ops + attribution dashboards' }
  }
]
...
export default function CrossbeamComparisonTable() {
  return (
    <section
            ...
            {ROWS.map((row) => (
              ...
              >
                ...
                <div className='flex items-center justify-center border-l border-[#e2e4eb] px-3 py-4'>
                  <CellContent cell={row.crossbeam} />
                </div>
