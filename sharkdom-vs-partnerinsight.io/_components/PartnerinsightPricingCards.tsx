...
const PARTNERINSIGHT_ITEMS = [
  { ok: false as const, label: 'Limited free trial' },
  { ok: true as const, label: 'GTM collaboration hubs' },
  { ok: false as const, label: 'No built-in LMS/onboarding' },
  { ok: false as const, label: 'No automated commission payouts' }
]

const SHARKDOM_ITEMS = [
  'Free 14-day trial & starter tier',
  'Flat pricing structure',
  'Built-in LMS + automated reward engine',
  'Unlimited partners & integrations'
]

export default function PartnerinsightPricingCards() {
  return (
        ...
        <header className='mx-auto flex max-w-4xl flex-col items-center text-center'>
          ...
          <h2 className='mt-10 max-w-[38rem] font-bold tracking-[-0.03em] text-[#111827]'>
            ...
            <span className='mt-1 block text-[clamp(1.875rem,4.5vw,3rem)] leading-[1.12] md:text-[48px] md:leading-[50px]'>
              <span className='text-[#7c71ff]'>GTM Teams</span>{' '}
              <span className='text-[#111827]'>vs flat rates</span>
            </span>
          </h2>
          ...
          <p className='mt-6 max-w-xl text-pretty text-base font-normal leading-relaxed text-[#6b7280] md:max-w-[36rem] md:text-[17px] md:leading-[26px]'>
            PartnerInsight.io pricing scales with partner collaboration volume,
            starting around $300/mo. Sharkdom offers flat, transparent billing
