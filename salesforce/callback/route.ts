...
export async function GET(req: Request) {
  ...
  console.log(
    ...
    process.env.NEXT_PUBLIC_SALESFORCE_CLIENT_ID ? '***' : 'MISSING'
  )
  console.log(
    '  - NEXT_PUBLIC_SALESFORCE_CLIENT_SECRET:',
    process.env.NEXT_PUBLIC_SALESFORCE_CLIENT_SECRET ? '***' : 'MISSING'
  )
  console.log(
    '  - NEXT_PUBLIC_SALESFORCE_REDIRECT_URL:',
    process.env.NEXT_PUBLIC_SALESFORCE_REDIRECT_URL ? '***' : 'MISSING'
  )
  console.log(
    '  - SHARKDOM_API_URL:',
  ...
  if (!process.env.NEXT_PUBLIC_SALESFORCE_CLIENT_ID) {
    console.error('❌ NEXT_PUBLIC_SALESFORCE_CLIENT_ID is missing')
    return NextResponse.json(
  ...
  if (!process.env.NEXT_PUBLIC_SALESFORCE_CLIENT_SECRET) {
    console.error('❌ NEXT_PUBLIC_SALESFORCE_CLIENT_SECRET is missing')
    return NextResponse.json(
      { error: 'Salesforce client secret not configured' },
      { status: 500 }
    )
  }

  if (!process.env.NEXT_PUBLIC_SALESFORCE_REDIRECT_URL) {
    console.error('❌ NEXT_PUBLIC_SALESFORCE_REDIRECT_URL is missing')
    return NextResponse.json(
      { error: 'Salesforce redirect URL not configured' },
      { status: 500 }
    )
  }
  ...
  // Ensure redirect URI is exactly what Salesforce expects
  const redirectUri = process.env.NEXT_PUBLIC_SALESFORCE_REDIRECT_URL ?? ''
  ...
  console.log('📤 Token exchange request data:', {
    grant_type: 'authorization_code',
    client_id: process.env.NEXT_PUBLIC_SALESFORCE_CLIENT_ID ? '***' : 'MISSING',
    client_secret: process.env.NEXT_PUBLIC_SALESFORCE_CLIENT_SECRET
      ? '***'
      : 'MISSING',
    redirect_uri: process.env.NEXT_PUBLIC_SALESFORCE_REDIRECT_URL
      ? '***'
      : 'MISSING',
    code: code ? '***' : 'MISSING'
  })
  ...
  try {
    ...
    if (!response.ok) {
      console.error('❌ Salesforce API error:', response.status, responseData)

      // Handle specific OAuth errors with user-friendly redirects
      if (
        responseData.error === 'invalid_grant' &&
        responseData.error_description?.includes('expired')
      ) {
        console.log(
      ...
      throw new Error(
        `Salesforce API error: ${response.status} - ${responseData.error || responseData.error_description || 'Unknown error'}`
      )
    }
    ...
    let redirectUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/integrations?success=salesforce_connected`
    console.log('🔄 Redirecting to:', redirectUrl)
    if (state) {
      const stateData = JSON.parse(decodeURIComponent(state))
      if (stateData?.source == 'my-data-flow') {
        redirectUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/my-data/connect-service/salesforce`
        console.log('🔄 Redirecting to:', redirectUrl)
      } else if (stateData?.source == 'partner-portal-flow') {
        redirectUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/partner-portal/partner-mapping/connect-service/salesforce`
        console.log('🔄 Redirecting to:', redirectUrl)
      }
    }
    return NextResponse.redirect(redirectUrl)
  } catch (error) {
    console.error('💥 Error during Salesforce OAuth callback:', error)
    console.error(
      '💥 Error stack:',
