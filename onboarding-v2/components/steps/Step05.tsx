...
const Step05 = () => {
  ...
  const buttons = ['B2B', 'B2B2C', 'B2C']

  const handleSelect = (index: number) => {
    updateStep('step5', buttons[index])
  }

  const getSelectedIndex = (): number | null => {
    if (!state.step5) return null
    const index = buttons.indexOf(state.step5)
      ...
      <div className='flex w-[60%] items-center justify-center bg-white px-10'>
        <div className='flex w-full max-w-md flex-col gap-6'>
          ...
          <h1 className='text-center text-2xl font-bold text-[#2A3241]'>
            Which Market segment do you want to partner in?
          </h1>
