// components/OnboardingCostFull.tsx
import React, { useState } from 'react'
import Image from 'next/image'
import Link from 'next/link'
import ConsistanceProcessIcon from '@/../public/icons/consistant-process.svg'
import ManualOnboardingIcon from '@/../public/icons/manual-onboarding.svg'
import PartnerConfIcon from '@/../public/icons/partner-confusion.svg'
import RevenuLossIcon from '@/../public/icons/revenu-loss.svg'
import RevenuProfitsIcon from '@/../public/icons/revenue-profit.svg'
import SharkDomAutomationIcon from '@/../public/icons/sharkdomautomation.svg'

import { Button } from '@/components/ui/button'

import {
  CurrentActiveStateIcon,
  CurrentInActiveStateIcon,
  StarActiveIcon,
  StarInActiveIcon
} from '../icons/icons'

export default function OnboardingCostFull() {
  const [variant, setVariant] = useState<'current' | 'sharkdom'>('sharkdom')

  return (
    <section className='relative mx-3 overflow-hidden rounded-2xl bg-[#F7F7FF] py-10 md:mx-5'>
      <div className='mx-auto max-w-7xl px-4 md:px-6'>
        <p className='mb-2 text-center text-3xl font-medium text-[#1E1E1E] lg:text-4xl'>
          Partner Onboarding,
          <span className='relative inline-block md:w-[310px] lg:w-[360px]'>
            <span className='relative z-10'>Refined for Revenue.</span>
            {/* decorative svg underline (kept from your file) */}
            <svg
        ...
        <div className='mb-8 flex justify-center gap-3'>
          <div className='mt-6 flex flex-col gap-4 md:flex-row'>
            <Button
              onClick={() => setVariant('sharkdom')}
              className={`rounded-full px-7 py-6 text-base font-semibold shadow-lg transition-all duration-300 hover:shadow-xl ${
                variant === 'sharkdom'
                  ? 'transform bg-gradient-to-r from-green-500 to-green-600 text-white hover:scale-105 hover:from-green-600 hover:to-green-700'
                  : 'transform border-2 border-green-500 bg-gradient-to-r from-green-50 to-green-100 text-green-600 hover:scale-105 hover:border-green-600 hover:from-green-100 hover:to-green-200'
              }`}
              variant={variant === 'sharkdom' ? 'default' : 'outline'}
            >
              <span className='pr-1'>
                {' '}
                {variant === 'sharkdom' ? (
            ...
            <Button
              ...
              className={`justify-center rounded-full px-7 py-6 text-base font-semibold transition-all duration-300 ${
                variant === 'current'
                  ? 'cursor-not-allowed bg-gray-400 text-gray-600 opacity-60'
                  : 'border border-gray-300 bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-600'
              }`}
        ...
        {/* large pale panel behind content */}
        <div className='md:p-6'>
          <div className='rounded-2xl bg-white py-12 md:px-6'>
            {/* visual row */}
            <div className='flex flex-col items-center justify-between gap-6 md:flex-row'>
              ...
              <div className='flex flex-1 flex-col items-center gap-3 text-center'>
                <div
                  className={`flex h-16 w-16 items-center justify-center rounded-full shadow-sm ${
                    variant === 'sharkdom' ? 'bg-[#FFF8F8]' : 'bg-[#FFF8F8]'
                  }`}
                >
                  {variant === 'sharkdom' ? (
                    <Image
                      src={SharkDomAutomationIcon}
                      alt='manual'
                      height={100}
                      width={100}
                    />
              ...
              <div className='hidden items-center px-6 md:flex'>
                <div className='text-5xl font-normal'>+</div>
              </div>

              {/* middle block */}
              <div className='flex flex-1 flex-col items-center gap-3 text-center'>
                <div className='flex h-16 w-16 items-center justify-center rounded-full bg-[#FFF8F8] shadow-sm'>
                  {/* <Image
                    src={PartnerConfIcon}
                    alt='partner-conf'
                    height={100}
              ...
              <div className='flex flex-1 flex-col items-center gap-3 text-center'>
                <div
                  className={`flex h-16 w-16 items-center justify-center rounded-full ${
                    variant === 'sharkdom'
                      ? 'bg-[#22C665]'
                      : 'border border-[#F3C1C1] bg-[#F43840]'
                  }`}
                >
                  {variant === 'sharkdom' ? (
                    <Image
                      src={RevenuProfitsIcon}
                      alt='manual'
                      height={25}
                      width={25}
                    />
                ...
                <div
                  className={`text-base font-medium ${variant === 'sharkdom' ? '' : 'text-[#EF4444]'}`}
                >
                  {variant === 'sharkdom'
                    ? '3X faster Growth'
                    : '40% Revenue Loss'}
            ...
            {/* description text */}
            <div className='mx-auto mt-10 max-w-3xl text-center text-base text-slate-500'>
              {variant === 'sharkdom' ? (
                <>
                  Sharkdom’s automated enablement platform creates consistent,
                  scalable partner experiences that drive predictable revenue
                  growth.
                </>
              ) : (
                <>
        ...
        {/* header for the stats section (changes text depending on variant) */}
        ...
        {/* four stat boxes - two styles */}
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 md:px-8'>
          {variant === 'sharkdom' ? (
            <>
              <div className='flex flex-col items-center justify-center rounded-2xl border border-[#5DF29A] bg-[#EDFFF4] p-4'>
                <div className='mb-1 text-center text-base font-medium'>
                  Partners complete onboarding successfully
                </div>
                <div className='text-center text-base font-medium text-[#198D49]'>
                  89% Completion Rate
                </div>
              </div>
              ...
              <div className='flex flex-col items-center justify-center rounded-2xl border border-[#5DF29A] bg-[#EDFFF4] p-4'>
                <div className='mb-1 text-center text-base font-medium'>
                  Increase in partner-driven revenue
                </div>
                <div className='text-center text-base font-medium text-[#198D49]'>
            ...
            <>
              <div className='flex flex-col items-center justify-center rounded-2xl border border-[#FE7D82] bg-[#FEF2F2] p-4'>
                <div className='mb-1 text-center text-base font-medium'>
                  Inconsistent partner training
                </div>
                <div className='text-center text-base font-medium text-[#F43840]'>
                  Lost deals worth $2.3M annually
                </div>
              </div>
              ...
              <div className='flex flex-col items-center justify-center rounded-2xl border border-[#FE7D82] bg-[#FEF2F2] p-4'>
                ...
                <div className='text-center text-base font-medium text-[#F43840]'>
                  45% partners never complete onboarding
                </div>
              </div>
              ...
              <div className='flex flex-col items-center justify-center rounded-2xl border border-[#FE7D82] bg-[#FEF2F2] p-4'>
                ...
                <div className='text-center text-base font-medium text-[#F43840]'>
                  3 weeks average time-to-productivity
                </div>
              </div>
            </>
          )}
