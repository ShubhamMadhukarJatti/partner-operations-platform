...
import { canSendProposals } from '../_components/utils/ProposalChecks'
...
const SendProposal = ({
  ...
  const [status, setStatus] = useState<string | null>(proposalStatus)
  ...
  const handleSendProposal = async () => {
    try {
      if (
        ...
        if (senderOrg.organizationCollaborations === null)
          setIsFirstPropsal(true)
        setScreen('sent')
      } else {
        setIsLoading(false)
      ...
      {status === null ? (
        ...
        >
          <AlertDialogTrigger asChild>
            <Button
              className='rounded-sm border  border-primary bg-primary px-7 text-primary text-white'
              size='sm'
            >
              Send Proposal
            </Button>
          </AlertDialogTrigger>
          {screen === 'sent' || screen === 'first-proposal' ? (
            <AlertDialogContent className='flex max-w-md flex-col items-center'>
              {screen === 'sent' && (
                <>
                  <Image
                  ...
                  <p className='max-w-sm py-2 text-center text-base leading-5 text-[#475467]'>
                    Proposal sent to {recieverOrg.name} successfully.{' '}
                    {recieverOrg.name} typically responds within 18 hours. You
                    will be notified via email once they respond.
                  </p>

                  <AlertDialogFooter className='mt-9 w-full'>
                    <Button
                          ...
                          <div className='space-y-2 p-2'>
                            ...
                            <div className='flex flex-col gap-4 pt-4'>
                              {expectations.map((expectation, index) => (
                                <div
                                  className='space-y-2'
                                  key={expectation.benefit}
                                >
                                  <div className='flex items-center justify-between'>
                                    <Label
                                      htmlFor={expectation.benefit}
                                      className=''
                                    >
                                      {expectation.benefit}
                                    </Label>
                      ...
                      <div className='text-muted-foreground'>
                        ...
                        <p className=' first-letter:uppercase'>
                          {(Array.isArray(
                            ...
                            recieverOrg?.preferredPartnershipTypes?.length >
                              0 &&
                            recieverOrg?.preferredPartnershipTypes
                              .map((type) => type?.area)
                              .join(', ')) ||
