...
const PartnerCourseOtpScreen = () => {
  ...
  // Redirect to partner-course login if no email/data
  if (!(email && data)) {
    router.push('/partner-course/login')
  }
  ...
  const handleVerify = async (
    ...
    setLoading(true)
    try {
      // Use partner login verify endpoint
      const res = await fetch('/api/login-verify-partner', {
        method: 'POST',
        headers: {
        ...
        <div className='mx-auto flex w-full max-w-md flex-col gap-8 pt-4'>
          ...
          <div className='flex w-full justify-between gap-6 pt-4'>
            <div className='mx-auto w-1/2 max-w-md'>
              <Link href='/partner-course/login'>
...
export default PartnerCourseOtpScreen
