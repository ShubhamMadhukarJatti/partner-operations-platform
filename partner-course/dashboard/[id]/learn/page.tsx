...
const CourseLearnPage = () => {
  ...
  const fetchProgress = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/partner-course/course/${courseId}/progress`)
    ...
    async (status: 'IN_PROGRESS' | 'COMPLETED') => {
      try {
        ...
        const res = await fetch(
          `/api/partner-course/course/${courseId}/status`,
          {
            method: 'PUT',
            headers: {
  ...
  const completeStage = useCallback(
    async (stageId: number) => {
      try {
        ...
        const res = await fetch(
          `/api/partner-course/course/${courseId}/stages/${stageId}/complete`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
  ...
  const allStagesCompleted = useMemo(() => {
    ...
    return chapters.every((ch) => completedStageIds.has(ch.id))
  }, [chapters, completedStageIds])

  // Check if current stage is completed
  const isCurrentStageCompleted = useMemo(() => {
    if (!currentStage) return false
    return completedStageIds.has(currentStage.stageId.toString())
  }, [currentStage, completedStageIds])
  ...
  const handleContinue = async () => {
    ...
    if (nextChapter && nextChapter.stageId) {
      ...
      // All stages completed - update course status to COMPLETED
      try {
                    ...
                    <div className='flex-1'>
                      <div
                        className={`text-sm font-semibold ${
                          chapter.isActive ? 'text-blue-900' : 'text-gray-900'
                        }`}
                      >
                      ...
                      <div
                        className={`mt-1 text-xs ${
                          chapter.isActive ? 'text-blue-700' : 'text-gray-600'
                        }`}
                      >
                        {chapter.title}
                      </div>
