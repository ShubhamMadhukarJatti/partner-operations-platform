...
import { CrownIcon } from '@/app/onboarding-v2.1/components/icons/icons'

import Pricing from './PricingCard'

export default function FreeTrailPayment({
  successUrl = '/explore',
  cancelUrl = '/free-trial/onboarding/payment'
}) {
  ...
  useEffect(() => {
    ...
    const getPricingData = async () => {
      const [trialPricing] = await Promise.all([
        getConfigByType('TRIAL_PRICING')
      ])
  ...
  const handleStripeUpgradePlan = async (planType: string) => {
    ...
    const session = await createStripeCustomerCheckout({
      ...
      mode: 'SUBSCRIPTION',
      trailDays: 14,
      successUrl: `${process.env.NEXT_PUBLIC_BASE_URL}${successUrl}`,
  ...
  return (
    <div className='flex h-full min-h-screen justify-center'>
      <div className='flex w-full flex-col bg-white'>
        {/* <div className='flex w-full flex-col bg-white'>
        <p className='flex items-center fds-text-lead-semibold'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <a
            ...
            className='flex cursor-pointer items-center gap-1 fds-text-sm text-[#3E50F7] hover:underline'
          >
        ...
        <div className='flex justify-center'>
          <div className='flex w-full flex-col justify-between border-b pt-4 md:px-4 md:pb-2'>
        ...
        <div className='flex flex-col items-center justify-center pt-6 text-center'>
          <CrownIcon />
          <p className='fds-text-lead-semibold w-full pt-2'>Choose Your Plan</p>
          <p className='px-4 py-2 text-base text-[#717182] md:w-[90%] md:px-2 lg:w-[55%]'>
            Select the perfect plan to power your partnership growth with
            Sharkdom with 14 days Full access
          </p>
        </div>
        ...
        <div className='w-full'>
          ...
          {/* Free Trial Section */}
          {/* <div className='flex h-full w-full flex-col justify-between rounded-[20px] border border-[#E4E7EE] bg-[#F8FBFF] text-black  shadow-lg lg:col-span-2 lg:w-[450px] '>
              <div>
                <div className='flex flex-col justify-between gap-2 rounded-tl-[20px] rounded-tr-[20px] bg-[#F2F5FD] p-6 text-left'>
                  <h3 className='flex items-center gap-4 fds-heading text-text-100'>
                    STANDARD
                    <span className='rounded-md bg-semantic-caution px-2 py-1  fds-text-sm text-text-100'>
                      FREE TRIAL
                    </span>
                  </h3>
                  ...
                  <div className='mt-6 flex items-center gap-3'>
                    <span className='text-2xl font-normal text-[#5E7193] line-through decoration-slate-500'>
                      {country === 'IN' ? '₹' : '$'}
                      {findWithoutDiscountValue(
                        standardPricing,
                        standardDiscount
                      )}
                    </span>

                    <p className='flex items-center gap-4 fds-heading text-text-100'>
                      {country === 'IN' ? '₹' : '$'}
                      {standardPricing}
                ...
                <ul className='my-12 p-2 px-6 text-left'>
                  ...
                  {country !== 'IN' && (
                    <li className='flex items-center gap-2 text-base font-normal'>
                      <span className='ml-7'>Cancel Any time</span>
                    </li>
                  )}
                </ul>
              </div>
              ...
              <div className='flex justify-center px-6 pb-6'>
                <Button
                  className='text-[#0062F1 w-full rounded-lg border border-[#ADB7CB] bg-transparent fds-text-semibold text-[#3E50F7] hover:text-white'
                  loading={
                    loading.planType === 'STANDARD_MONTHLY' && loading.loading
                  }
          ...
          {/* <div className='flex h-full w-full flex-col justify-between rounded-[20px] border border-[#CCE0FC] bg-[#F8FBFF] text-black shadow-lg  lg:col-span-2 lg:w-[450px]'>
              <div>
                <div
                  style={{
                    background:
                      'linear-gradient(90deg, #E5EFFE 0%, #FFFFFF 100%)'
                  }}
                  className='flex flex-col justify-between gap-2 rounded-tl-[20px] rounded-tr-[20px] p-6 text-left'
                >
                  <div className='flex gap-4'>
                    <h3 className='fds-heading text-text-100'>
                      Premium
                    </h3>
                    <p className='w-fit rounded-lg bg-white px-3 py-2 fds-text-sm text-[#2A3241]'>
                      POPULAR
                    </p>
                  </div>
                  <span className='text-shark-sm text-text-100'>
                  ...
                  <div className='mt-6 flex items-center gap-3'>
                    ...
                    <p className='flex items-center gap-4 fds-heading text-text-100'>
                      <span>
                        ...
                        <span className='fds-text text-[#252424]'>
                          /month
                        </span>
                      </span>
                      <p className='flex items-baseline fds-text-semibold text-[#488D12]'>
              ...
              <div className='flex justify-start px-6 pb-6'>
                <Button
                  loading={
                    loading.planType === 'PREMIUM_MONTHLY' && loading.loading
                  }
                  className=' w-full rounded-lg border border-[#ADB7CB] bg-[#3E50F7] fds-text-semibold text-white '
          ...
          <div className='space-y-6'>
            <div className='space-y-4 bg-[#F2F5FD] text-center'>
              <h2 className='fds-heading'>Standard</h2>
          ...
          <div className='space-y-6'>
            <div className='space-y-4 text-center'>
              <h2 className='fds-heading'>Premium</h2>
